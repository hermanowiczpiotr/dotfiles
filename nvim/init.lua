...
require("nvim-tree").setup({
  ...
  filters = {
    ...
    if api and api.node and api.node.open and api.node.open.vertical then
      vim.keymap.set('n', 'v', api.node.open.vertical, { buffer = bufnr, noremap = true, silent = true })
    end

    if api and api.node and api.node.open and api.node.open.horizontal then
      vim.keymap.set('n', 's', api.node.open.horizontal, { buffer = bufnr, noremap = true, silent = true })
    end
    ...
    if api and api.fs and api.fs.mkdir then
      vim.keymap.set('n', 'd', api.fs.mkdir, { buffer = bufnr, noremap = true, silent = true })
    end
  end
})
